...
// When the time comes to insert a value, each node adds additional storage
// that determines whether to perform <= (left) or >= (right)

public class AlternateEqualBST<Key extends Comparable<Key>> {
	...
	class Node {
		...
		boolean eqlRight;     // go right if equal
	...
	private Node insert(Node parent, Key key) {
		...
		} else if (cmp == 0 && parent.eqlRight) {
			parent.eqlRight = !parent.eqlRight;
			parent.right = insert(parent.right, key);
		} else {
			parent.eqlRight = !parent.eqlRight;
			parent.left  = insert(parent.left,  key);    // alternate on equality....
		}
